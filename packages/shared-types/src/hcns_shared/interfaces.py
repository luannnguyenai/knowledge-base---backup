...
class PipelineAdapter(Protocol):
    """Adapter boundary between the eval framework and any concrete pipeline.

    The eval harness depends ONLY on this Protocol — it never imports a concrete
    pipeline (StaticPipeline, LangGraphAgent) directly. Adapters live in the eval
    package's `adapters/` module and wrap a pipeline built from a BenchmarkConfig.

    index -- ingest a corpus directory and build the index
    query -- answer a single question end-to-end, returning an Answer
    """
