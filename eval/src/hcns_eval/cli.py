...
def _build_from_config(cfg: "BenchmarkConfig"):  # type: ignore[name-defined]
    """Build a PipelineAdapter for *cfg* via the adapter layer.

    The CLI/harness only ever touch the adapter (index/query), never a concrete
    pipeline — keeping eval pipeline-agnostic.
    """
    from hcns_eval.adapters import build_adapter
...
def compare(
    ...
    from hcns_eval.harness import run_full_eval
    from hcns_eval.judge import Judge
    from hcns_eval.report import CompareReport, compare_runs
