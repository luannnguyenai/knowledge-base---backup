...
def build_pipeline_from_config(config: "BenchmarkConfig") -> StaticPipeline:  # type: ignore[name-defined]
    ...
    # Trigger @register decorators: backend components + agent generators
    import hcns_backend  # noqa: F401  (registers parser/chunker/embedder/vstore/retriever/reranker)
    import hcns_agents.generators  # noqa: F401  (registers fake/gemini/routed generators)

    from hcns_shared.registry import build
...
def _build_generator(spec: dict) -> object:
    ...
    from hcns_agents.generators import RoutedGenerator  # noqa: PLC0415
