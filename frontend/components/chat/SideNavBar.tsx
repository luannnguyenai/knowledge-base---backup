...
import logoImage from '@/assets/logo/logo-short.png';

export function SideNavBar() {
  const pathname = usePathname();
  const getNavClasses = (href: string) => {
    const isActive = pathname === href;
    return `flex items-center space-x-3 px-4 py-3 rounded-lg font-semibold active:scale-95 duration-150 transition-colors ${isActive
    ...
    <nav className="w-[280px] h-full flex flex-col z-40 bg-white/[0.03] backdrop-blur-xl border-r border-black/10 dark:border-white/10 shrink-0 hidden md:flex">
      <div className="px-6 py-4 flex flex-col gap-1 mb-4 h-16 justify-center">
        <div className="flex items-center justify-start py-2 gap-3">
          <Image
            alt="Vinsmart Future"
            className="h-4 w-auto object-contain brightness-0 dark:invert opacity-90"
            src={logoImage}
            width={100}
            height={20}
            priority
          />
          <span className="font-bold text-md tracking-tight text-[#1b1b1b] dark:text-white">HR Portal</span>
        </div>
      </div>
